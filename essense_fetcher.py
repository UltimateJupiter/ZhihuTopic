...
def get_raw_json(topicid, limit, offset, q_count):
    ...
    for i in range(limit):
        try:
            vals = data[i]['target']
            qvals = vals['question']
            info = [vals['id'], vals['created_time'], vals['voteup_count'], vals['comment_count'], qvals['created'], qvals['id'], qvals['title']]
            q_id = vals['id']
            res.append(info)
        except:
...
def get_topic_essence(topic_id, q_count):
    print("Scrapping {} with {} questions".format(topic_id, q_count))
    res = Parallel(n_jobs=1, verbose=11)(delayed(get_raw_json)(topic_id, 10, 10 * i, q_count) for i in range(int(q_count / 10)))
    return res

if __name__ == '__main__':
    topic_id = 19582963
    res = get_topic_essence(19582963, 10)
    print(res)
